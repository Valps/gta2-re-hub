...
* Open IDA database of GTA2 v10.5
* Pick a non-black colored function (non-matched function)
* Copy the function name you want to match.
* Create a decompme scratch using `generate_function_decompme.py`. For example go to `Scripts` folder and run

```
python generate_function_decompme.py sub_4B9BC0
```
...
* Put local variables inside the scopes in which they are used (if possible). In other words, avoid leaving lots of locals defined on the beginning of the function if possible.
